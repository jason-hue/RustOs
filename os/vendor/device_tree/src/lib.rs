...
use alloc::vec::Vec;
use core::{slice, str};
use util::{align, SliceRead, SliceReadError, StringList, VecWrite, VecWriteError};
...
impl Node {
    ...
    pub fn prop_str_list<'a>(&'a self, name: &str) -> Result<StringList<'a>, PropError> {
        let raw = self.prop_raw(name).ok_or(PropError::NotFound)?;
        Ok(StringList::new(raw))
    }
    ...
    pub fn prop_cells(&self, name: &str) -> Result<Vec<u32>, PropError> {
        let raw = self.prop_raw(name).ok_or(PropError::NotFound)?;

        Ok(raw.as_slice().read_be_cells()?)
    }
