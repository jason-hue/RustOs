use crate::ext4fs::prelude::*;
use crate::return_errno_with_message;
use crate::ext4fs::utils::Ext4Error;
use crate::ext4fs::ext4_defs::*;
...
impl Ext4 {
    ...
    pub fn dir_get_entries(&self, inode: u32) -> Vec<Ext4DirEntry> {
        ...
        // load inode
        let inode_ref = self.get_inode_ref(inode);//根目录的inode ref
        /*
        Ext4InodeRef {
            inode_num: inode_num,
            inode: *inode,
        ...
        while iblock < total_blocks {
            ...
            let search_path = self.find_extent(&inode_ref, iblock as u32); //查找逻辑块对应的物理块
            ...
            if let Ok(path) = search_path {
                ...
                // load physical block
                let ext4block =
                    Block::load(self.block_device.clone(), fblock as usize * BLOCK_SIZE);//从块设备加载物理块
                let mut offset = 0;//初始化块内偏移
