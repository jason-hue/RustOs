...
impl Ext4 {
    ...
    pub fn open(block_device: Arc<dyn BlockDevice>) -> Self {
        // Load the superblock
        let block = Block::load(block_device.clone(), SUPERBLOCK_OFFSET);//block存的超级块数据
        let super_block: Ext4Superblock = block.read_as();//读取超级块
    ...
    pub fn dir_mk(&self, path: &str) -> Result<usize> {
        ...
        let r = self.generic_open(path, &mut parent, true, filetype.bits(), &mut nameoff);//打不开就创建吧。。。。
        Ok(EOK)
    }
