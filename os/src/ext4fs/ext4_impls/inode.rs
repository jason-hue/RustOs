...
impl Ext4 {
    ...
    pub fn get_inode_ref(&self, inode_num: u32) -> Ext4InodeRef {
        let offset = self.inode_disk_pos(inode_num);//获得inode号在磁盘的偏移量

        let mut ext4block = Block::load(self.block_device.clone(), offset);

        let inode: &mut Ext4Inode = ext4block.read_as_mut();//将加载的数据块解释为Ext4Inode可变引用
